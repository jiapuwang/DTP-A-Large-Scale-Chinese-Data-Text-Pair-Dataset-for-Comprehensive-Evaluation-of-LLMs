"""
CDTP — Shared Prompt Templates
统一提示词模板库，供 base/ 和 sft/ 下所有推理脚本使用。
"""

# =============================================================================
# 示例问答（Few-shot examples for QA task）
# =============================================================================
_QA_EXAMPLES = (
...
# =============================================================================
# 示例文本生成（Few-shot examples for Text Generation task）
# =============================================================================
_TEXT_GEN_EXAMPLES = (
...
# =============================================================================
# 示例知识图谱补全（Few-shot examples for KG Completion task）
# =============================================================================
_KG_EXAMPLES = (
...
def build_qa_prompt(data: dict) -> str:
    """构建问答任务提示词。"""
    return _QA_EXAMPLES + "问题：" + data["response"] + "，答案："


def build_text_gen_prompt(data: dict) -> str:
    """构建文本生成任务提示词。"""
    import re
    triple_str = ""
    for element in data.get("triples", []):
        parts = element.split("\t")
        if len(parts) < 3:
            raise ValueError(f"Triple 字段格式错误，期望 3 个字段，实际：{parts}")
        triple_str += f"({parts[0]},{parts[1]},{parts[2]}),"
    triple_str = re.sub(r'\)\s*,$', ')', triple_str)
    return _TEXT_GEN_EXAMPLES + triple_str + "] 生成的文本："


def build_kg_prompt(data: dict) -> str:
    """构建知识图谱补全任务提示词。"""
